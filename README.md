...
> **Investigación aplicada en visión por computadora.** Pipeline de producción que transforma un flujo de webcam en inteligencia operativa: detecta personas, asigna identidades persistentes, cuenta cruces por línea virtual y produce analítica espacial y temporal post-sesión — todo a 20 FPS sobre CPU Apple Silicon.
...
| Nivel | Capacidades |
|---|---|
| **Notebook básico** | Detección de clase `person` · FPS · latencia de inferencia · cantidad por frame · confianza promedio · exportación a CSV |
| **Notebook profesional** | Lo anterior + **tracking persistente con ByteTrack** · conteo bidireccional por línea virtual · grabación de video anotado en MP4 · sistema de alertas configurables con cooldown |
| **Bloque 14 (aporte de investigación)** | Re-inferencia post-sesión sobre el MP4 generado para construir **heatmap espacial de zonas transitadas** + **dwell-time real por identificador único** — métricas estándar en analítica de retail y movilidad humana |

Este trabajo se desarrolla en el marco del curso **Bases de Datos II** de la **Universidad Autónoma de Occidente (UAO)** como práctica técnica trasladable al proyecto integrador **SICTL — Sistema de Identificación y Clasificación Taxonómica de Lepidópteros**, donde los mismos principios de detección, tracking persistente y registro a base de datos no relacional (MongoDB) constituirán el núcleo del sistema final.

### Hallazgos clave de la investigación

- **YOLOv8n alcanza 20 FPS sostenidos sobre CPU Apple M5** sin necesidad de aceleración GPU/Neural Engine, lo que demuestra la viabilidad de despliegues edge sobre dispositivos de consumo.
- **El filtrado de clase nativo en `model.track(classes=[0])` mejora el rendimiento +68% vs el flujo de inferencia genérico**, al evitar el NMS sobre clases descartadas.
- **ByteTrack mantiene identidades persistentes con re-asociación robusta**: 46 identificadores únicos a lo largo de 2 416 frames con permanencia individual de hasta 82 segundos.
- **La latencia escala linealmente con la densidad de la escena**: de 30 ms con 1-2 personas a 80 ms con 5-8 personas, comportamiento esperado en arquitecturas anchor-free.
...
> El video `salida_deteccion_personas_profesional.mp4` (114 MB) **no se versiona en GitHub** por exceder el límite de tamaño por archivo (100 MB). Se regenera ejecutando el notebook profesional.
...
El profesor entregó únicamente los notebooks base/profesional. Como aporte del autor se diseñó un módulo de **analítica post-sesión** que opera sobre el archivo MP4 generado por el cuaderno profesional, re-inferenciando con `model.track(persist=True)` para obtener centroides y trayectorias persistentes. Los resultados se materializan en dos visualizaciones complementarias:
...
> Este repositorio forma parte del portafolio académico del autor como investigación sobre el potencial de los modelos de detección en tiempo real aplicados a casos de uso de bases de datos y analítica.
